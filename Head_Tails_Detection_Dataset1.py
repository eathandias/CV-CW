...
def process_image(model, input_image_path, output_image_path, threshold=0.5, confidence_value_threshold=0.625):
    # Load and preprocess image
    input_image = Image.open(input_image_path).convert("RGB")
    transform = T.Compose([T.ToTensor()])
    image_tensor = transform(input_image).unsqueeze(0)  # [1, C, H, W]
    ...
    # Extract predictions (Faster R-CNN does not output masks)
    ...
    class_names = {0: "background", 1: "Head", 2: "Tail"}

    head_count = 0
    details_list = []

    # Process each detection: draw bounding boxes and count Heads
    for i in range(len(boxes)):
        x1, y1, x2, y2 = map(int, boxes[i])
        # Draw bounding box (using white borders)
...
def plot_head_distribution(head_counts, dataset_name="Dataset"):
    print("Head counts:", head_counts)  # Debug print

    plt.figure(figsize=(8, 6))
    
    # Handle case when all head_counts are the same
    if min(head_counts) == max(head_counts):
        bins = [min(head_counts) - 0.5, max(head_counts) + 0.5]
    else:
...
def main():
    ...
    model_path = "head_tail_coin_final.pth"  
    
    # Set paths for the dataset
    input_folder = r"C:\Users\eatha\Downloads\dataset2_2025ECPb\dataset1"
    output_folder = r"C:\Users\eatha\Downloads\dataset2_2025ECPb\output\Head_Tail\dataset1"
    
    # Load the model using Faster R-CNN
    model = get_fasterrcnn_model(num_classes)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.to(device)
    
    # Process dataset and get head counts per image
    ...
    plot_head_distribution(head_counts, dataset_name="Dataset1")
